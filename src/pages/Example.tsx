...
export default function ExamplePage() {
  ...
  const initialNodes: Node<CustomNodeData>[] = useMemo(() => {
    return exampleData.nodes.map((node: { id: string; position: { x: number; y: number }; data: { label: string; status: string; resources?: { enlaces?: { nombre: string; url: string }[] } } }) => ({
      ...
      data: { label: node.data.label, status: node.data.status, isEditing: false, resources: node.data.resources },
    }))
  }, [])
  ...
  return (
    <div className="w-full h-screen flex flex-col" style={{ backgroundColor: 'var(--color-surface)' }}>
      ...
      <div className="absolute top-4 left-4 z-20 flex gap-2 flex-wrap">
        <button
          onClick={() => window.open('/example-viewer', '_blank')}
          ...
          <div className="mb-4">
            ...
            <select
              ...
              style={{ 
                ...
                color: statusColors[clickedNode.data.status as keyof typeof statusColors] || statusColors.pendiente,
