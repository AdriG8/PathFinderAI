  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage('Instrucciones enviadas a tu email')
      }
    } catch (err) {
      ...
      <main className="flex-grow flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-xl flex flex-col items-center">
          <div className="mb-12 flex flex-col items-center justify-center">
            <img alt="PathFinderAI Logo" className="h-20 w-auto mb-4" src="/Picture.png" />
          ...
          <div className="w-full p-8 md:p-12 rounded-lg shadow-2xl" style={{ backgroundColor: 'var(--color-surface-container-low)' }}>
            <div className="text-center mb-10">
              ...
              <p className="text-sm leading-relaxed max-w-xs mx-auto" style={{ color: 'var(--color-on-surface-variant)' }}>
                Introduce tu email para recibir las instrucciones de recuperación.
              </p>
            </div>

            {message ? (
              <div className="text-center p-4 rounded-xl" style={{ backgroundColor: '#10b98120', color: '#10b981' }}>
                {message}
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="pt-4 flex justify-center">
                  <button 
                    className="font-bold px-10 py-4 rounded-full flex items-center gap-3 transition-transform hover:scale-[1.02] active:scale-95 shadow-lg w-2/3 justify-center mx-auto disabled:opacity-50"
                    style={{ backgroundColor: 'var(--color-surface-bright)', color: 'var(--color-on-surface)' }}
                    type="submit"
                    disabled={loading}
                  >
                    {loading ? 'Enviando...' : 'Enviar instrucciones'} <span className="material-symbols-outlined text-[18px]">arrow_forward</span>
                  </button>
                </div>
              </form>
            )}
